...
| `backend/`                   | Aplicación FastAPI: routers, servicios, repositorios, modelos | Para implementar lógica y API |
| `backend/routers/`           | Endpoints FastAPI organizados por dominio                 | Para implementar rutas API |
| `backend/models/`            | Modelos Pydantic (dominio y schemas de request/response)  | Al definir tipos de datos |
| `backend/services/`          | Lógica de negocio y autenticación                         | Para implementar reglas de dominio |
| `backend/storage.py`         | Persistencia JSON en disco (lectura/escritura atómica)    | Para implementar persistencia |
| `data/`                      | Datos estáticos de referencia (catalog.json, products.py) | Al trabajar catálogo |
| `components/`                | Componentes React del sitio público                       | Para implementar UI |
| `app/`                       | Rutas Next.js públicas (frontend)                         | Para implementar UI pública |
| `tests/`                     | Tests pytest (`pytest`)                                   | Para verificar |
| `scripts/`                   | Scripts de seed y utilidades de mantenimiento             | Al migrar datos |
